...
# EconomicBridge — reset Postgres 16 'postgres' user password to 'devpassword'.
#
# Same pattern as reset_postgres_password.ps1 but targets the v16 install
# on port 5434 (matches project canon — CLAUDE.md / memory snapshot).
#
# How to run:
#   1. Right-click Windows PowerShell -> Run as Administrator
#   2. powershell -ExecutionPolicy Bypass -File "c:\Users\HP\Downloads\economicbridge-ide-starter\economic-bridge-project\scripts\reset_postgres16_password.ps1"
...
try {
    ...
    & $psql -U postgres -h 127.0.0.1 -p $port -d postgres -c "ALTER USER postgres WITH PASSWORD '$newPw';"
...
Write-Host "[7/7] Verifying new password on port $port"
$env:PGPASSWORD = $newPw
& $psql -U postgres -h localhost -p $port -d postgres -c "SELECT 'reset OK' AS status, current_user, current_database();"
if ($LASTEXITCODE -eq 0) {
    Write-Host ""
    Write-Host "DONE. Postgres 16 'postgres' password is now '$newPw' (port $port)." -ForegroundColor Green
